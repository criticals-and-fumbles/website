  ...
  return (
    <section className="relative w-full py-20 border-t border-gold-500/20 max-w-[1500px] mx-auto" id="chronicles">
      ...
      <div className="text-center max-w-2xl mx-auto mb-10">
        <h2 className="font-serif text-4xl sm:text-5xl font-semibold text-[#f8f5ee] mb-3">Chronicles &amp; Lore</h2>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2.5 sm:gap-4 mb-12">
        ...
        {["DM Guides", "Worldbuilding", "Rule Systems"].map((label) => (
          <button
            key={label}
            className="px-4 py-1.5 rounded-full text-xs font-cinzel tracking-[0.16em] uppercase bg-[#08131e]/80 text-gray-400 border border-gold-500/25"
          >
            {label}
          </button>
        ))}
      ...
      <ScrollReveal className="grid grid-cols-1 lg:grid-cols-3 gap-7">
        {articles.map((a) => (
          <Link
            ...
            className="ornate-card corner-notch rounded-lg p-6 sm:p-7 bg-[#0b141f]/85 backdrop-blur-md flex flex-col justify-between group hover:border-gold-400 transition-all duration-300"
          >
            <div>
              ...
              <h3 className="font-serif text-2xl font-semibold text-gray-100 group-hover:text-gold-200 transition-colors leading-snug mb-3">
                {a.title}
              </h3>
              {a.excerpt && (
                <p className="font-serif text-gray-300 text-base leading-relaxed mb-6 line-clamp-3">{a.excerpt}</p>
              )}
            </div>
            <div className="pt-4 border-t border-gold-500/20 flex items-center justify-between mt-auto">
              {a.author && (
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full border border-gold-500/60 bg-[#061e2b] flex items-center justify-center text-gold-300 font-cinzel text-xs font-bold shadow-[0_0_8px_rgba(212,175,55,0.3)]">
                    {a.author.handle.slice(0, 2).toUpperCase()}
                  </div>
                  <span className="text-xs font-cinzel tracking-wider text-gray-200">{a.author.handle}</span>
                </div>
              )}
              <span className="inline-flex items-center gap-1.5 text-xs font-cinzel tracking-[0.18em] text-gold-300 group-hover:text-yellow-200 group-hover:translate-x-1 transition-all uppercase">
                <span>Read</span>
                <span>→</span>
              </span>
            </div>

...
export async function generateMetadata({
  params,
}: PageProps<"/wiki/[world]/factions/[slug]">): Promise<Metadata> {
  const { world: worldSlug, slug } = await params;
  const faction = await client.fetch<Faction | null>(FACTION_QUERY, { slug, worldSlug });
  if (!faction) return {};
  ...
  return buildMetadata({
    ...
    path: `/wiki/${worldSlug}/factions/${slug}`,
    image: urlForImage(faction.banner)?.width(1200).height(630).url(),
  });
}

export default async function WorldFactionPage({
  params,
}: PageProps<"/wiki/[world]/factions/[slug]">) {
  const { world: worldSlug, slug } = await params;
  const faction = await client.fetch<Faction | null>(FACTION_QUERY, { slug, worldSlug });
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_240px] lg:items-start">
              ...
              <aside className="mt-10 border-t border-border pt-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {faction.members.map((member) => (
                    <Link
                      key={member._id}
                      href={`/wiki/${worldSlug}/figures/${member.slug}`}
          ...
          {faction._createdAt && faction._updatedAt && (
            <WikiEntryMetaPanel
              ...
              parentLink={{ label: faction.world?.name ?? worldSlug, href: `/wiki/${worldSlug}` }}

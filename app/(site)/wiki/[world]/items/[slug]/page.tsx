...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    path: `/wiki/${worldSlug}/items/${slug}`,
...
export default async function WorldMagicItemPage({
  params,
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_240px] lg:items-start">
          <article>
            ...
            <div className="mt-10 flex flex-wrap gap-3 border-t border-border pt-6 font-ui text-xs text-text-muted">
              {item.currentHolder && (
                <Link
                  href={`/wiki/${worldSlug}/figures/${item.currentHolder.slug}`}
              ...
              {item.foundAt && (
                <Link
                  href={`/wiki/${worldSlug}/places/${item.foundAt.slug}`}
                  className="hover:text-emerald"
                >
